...
def play_wav(wav, blocking=True):
    try:
        ...
        # make up for that
        wav = np.concatenate((wav, np.zeros(hparams.sample_rate // 2)))
        sd.play(wav, hparams.sample_rate, blocking=blocking)
...
def interactive_diarization(similarity_dict, wav, wav_splits, x_crop=5, show_time=False):
    ...
    times = [((s.start + s.stop) / 2) / hparams.sample_rate for s in wav_splits]
